...
В каждый конкретный момент времени человек смотрит на мир через призму определенной системы понятий, и прежде чем формулировать какую-либо проблему, нам придется принять какую-то модель.
И для этого нам нужно прийти к единому набору понятий (в зависимости от разных точек зрения (viewpoint) реальность может описываться разными системами понятий), терминов для моделирования.
